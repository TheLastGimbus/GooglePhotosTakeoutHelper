...
Future<Directory> getInputDir() async {
  ...
  final String? dir =
      await getDirectoryPath(dialogTitle: "Select unzipped folder:");
  await sleep(1);
  if (dir == null) {
...
Future<Directory> getOutput() async {
  ...
  pressEnterToContinue();
  final String? dir =
      await getDirectoryPath(dialogTitle: "Select output folder:");
  await sleep(1);
  if (dir == null) {
    error("Duh, something went wrong with selecting - try again!");
    return getOutput();
...
/// Ask if user wants to skip GPS extraction (for faster processing)
Future<bool> askSkipGps() async {
  print("Do you want to extract GPS data from JSON files?");
  print("This can be useful if you want to preserve location information,");
  print("but it will make processing slower for large collections.");
  print("");
  print("1) Yes, extract GPS data (recommended)");
  print("2) No, skip GPS extraction (faster)");
  print("");
  stdout.write("Your choice (1/2): ");
  final String choice = stdin.readLineSync()?.trim() ?? "1";

  // Return true if user wants to SKIP GPS extraction
  return choice == "2";
}
